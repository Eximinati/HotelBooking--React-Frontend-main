...
const registerUser = async (req, res) => {
  try {
    ...
    if (firstname.length < 3 || firstname.length > 30) {
      return res.status(400).json({ msg: "First name must be between 3 and 30 characters." });
    }
    if (lastname.length < 3 || lastname.length > 30) {
      return res.status(400).json({ msg: "Last name must be between 3 and 30 characters." });
    }
    ...
    if (address.length < 5 || address.length > 100) {
      return res.status(400).json({ msg: "Address must be between 5 and 100 characters." });
    }
    if (!/^03\d{9}$/.test(phone)) {
      return res.status(400).json({ msg: "Phone must start with 03 and be 11 digits." });
    }
...
// Get max admin limit from .env or default to 15
const MAX_ADMIN_LIMIT = parseInt(process.env.MAX_ADMIN_LIMIT) || 15;

if (role === "admin") {
  if (allowedAdminEmails.includes(email.toLowerCase()) && adminCount < MAX_ADMIN_LIMIT) {
    assignedRole = "admin";
  } else {
    ...
    // Create user with error handling for Mongoose validation
    let newUser;
    try {
